...
import { Send, Trash2, CheckCircle2, Eye } from 'lucide-react'
...
export default function PostCard({ post, onDelete, onPublish, onPreview, selected, onToggleSelect }: PostCardProps) {
  ...
  return (
    <div className={cn('bg-white rounded-card shadow-card p-4 transition-shadow', selected && 'ring-2 ring-brand-azure')}>
      <div className="flex items-start gap-3">
        {onToggleSelect && (
          <input
            ...
            className="mt-0.5 h-4 w-4 flex-shrink-0 rounded border-gray-300 text-brand-azure focus:ring-brand-azure cursor-pointer"
          />
        )}
        {post.imageUrl && (
        ...
        <div className="flex-1 min-w-0">
          ...
          <button type="button" onClick={onPreview} className="block w-full text-left">
            <p className="text-xs text-gray-600 line-clamp-2 hover:text-brand-navy transition-colors">{post.caption}</p>
          </button>
      ...
      <div className="flex items-center gap-3 mt-3 pt-3 border-t border-gray-100">
        <button type="button" onClick={onPreview}
          className="flex items-center gap-1.5 text-[11px] font-semibold text-gray-500 hover:text-brand-navy transition-colors">
          <Eye size={11} /> Preview
        </button>
        {post.status !== 'PUBLISHED' && (
          <>
            ...
            <button type="button" onClick={() => { if (window.confirm('Delete this scheduled post?')) onDelete() }}
        ...
        {post.status === 'PUBLISHED' && (
          <>
            {post.platformPostId && (
              <span className="text-[10px] text-green-600 flex items-center gap-1"><CheckCircle2 size={10} /> {post.platformPostId}</span>
            )}
            <button type="button" onClick={() => { if (window.confirm('Delete this post from the scheduler?')) onDelete() }}
              className="flex items-center gap-1 text-[11px] text-gray-400 hover:text-red-500 transition-colors ml-auto">
              <Trash2 size={11} />
            </button>
          </>
